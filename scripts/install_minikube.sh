...
function install_oc() {
  if ! [ -x "$(command -v oc)" ]; then
    echo "Installing oc..."
    curl -Lo oc.tar.gz https://mirror.openshift.com/pub/openshift-v4/clients/oc/${OPENSHIFT_VERSION:-4.4}/linux/oc.tar.gz && sudo tar -C /usr/local/bin/oc -xf oc.tar.gz && rm -f oc.tar.gz
  else
